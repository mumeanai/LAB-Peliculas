...
def actores_mas_frecuentes(peliculas:list[Pelicula], n:int, año_inicial:date.year = None, año_final:date.year = None):
    '''
    ...
    #         for r in p.reparto:
    #             res[r].append(p)
    # actores_peliculas=sorted(res.items(), key=lambda t:len(t[1]))[:n]
    # actores = [actor for actor, pelicula in actores_peliculas]
    # return actores
    
    res = peliculas_por_actor(peliculas, año_inicial, año_final)
    ordenada = sorted(res.items(), key= lambda r:r[1], reverse= True)[:n]
    return sorted([actor for actor, num_peliculas in ordenada])
...
def recaudacion_total_por_año(peliculas:list[Pelicula], generos:set[str] = None) -> dict[date.year:int]:
    '''
    ...
    for p in peliculas:
        if generos == None:
            res[p.fecha_estreno.year] += p.recaudacion
        elif  set(p.generos) & generos:
            res[p.fecha_estreno.year] += p.recaudacion   
            
    return res
    

def incrementos_recaudacion_por_año(peliculas:list[Pelicula], generos:set[str] |None = None) -> list[int]:
    '''
    ...
    (1,5 punto)
    '''
    res = recaudacion_total_por_año(peliculas, generos)
    ordenado_años = sorted(res.items())
    diferencias = []
    for e1, e2 in zip(ordenado_años, ordenado_años[1:]):
        diferencias.append(e2[1]-e1[1])
    return diferencias
